// Package config loads Audio Analysis Worker settings from the environment.
package config
...
const ServiceName = "audio-analysis-worker"

// ConsumerGroup consumes media.events.
const ConsumerGroup = "audio-analysis-worker"
...
type Config struct {
	...
	FFmpegPath   string
	// AnalysisTimeout bounds one track; presigned URLs live as long.
...
func Load() (Config, error) {
	...
	cfg := Config{
		...
		AnalysisTimeout: env.Duration("ANALYSIS_TIMEOUT", 5*time.Minute),
		Tracing: telemetry.TracingConfig{
	...
	if cfg.AnalysisTimeout <= 0 {
		return cfg, errors.New("ANALYSIS_TIMEOUT must be positive")
	}
	return cfg, nil
}

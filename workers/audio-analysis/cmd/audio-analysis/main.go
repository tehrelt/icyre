// Command audio-analysis measures audio features of uploaded masters: it
// consumes track.uploaded from media.events, decodes the master with
// ffmpeg, measures tempo, loudness and silence, stores them and publishes
// audio.features_extracted through its outbox
// (specs/workers/audio-analysis.md).
//
//	audio-analysis          consume media.events
//	audio-analysis migrate  apply database migrations and exit
package main
...
import (
	...
	"github.com/tehrelt/icyre/workers/audio-analysis/internal/adapters/ffmpeg"
	kafkaadapter "github.com/tehrelt/icyre/workers/audio-analysis/internal/adapters/kafka"
	pgadapter "github.com/tehrelt/icyre/workers/audio-analysis/internal/adapters/postgres"
	"github.com/tehrelt/icyre/workers/audio-analysis/internal/adapters/storage"
	"github.com/tehrelt/icyre/workers/audio-analysis/internal/application"
	"github.com/tehrelt/icyre/workers/audio-analysis/internal/config"
)

func main() {
	if err := run(os.Args[1:]); err != nil {
		fmt.Fprintln(os.Stderr, "audio-analysis:", err)
		os.Exit(1)
	}
}
...
func run(args []string) error {
	...
	analyzer := ffmpeg.New(cfg.FFmpegPath)
	if err := analyzer.Version(ctx); err != nil {
	...
	app := application.New(storage.New(store, cfg.AnalysisTimeout), analyzer, repo, log)
	...
	}, kafkaadapter.Handler(app, kafkaadapter.NewMetrics(reg), cfg.AnalysisTimeout), log, platformkafka.NewConsumerMetrics(reg))
	if err != nil {
		return err
	}
	...
	log.Info("audio analysis worker stopped")
	return nil
}

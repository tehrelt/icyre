// Package config loads Listening History settings from the environment.
package config
...
const ServiceName = "listening-history-service"

// ConsumerGroup consumes playback.events.
const ConsumerGroup = "listening-history"
...
type Config struct {
	...
	JWKSURL string
	// Rule decides which plays count as listens.
...
func Load() (Config, error) {
	...
	cfg := Config{
		...
		MinListen:      env.Duration("HISTORY_MIN_LISTEN", 30*time.Second),
		MinFraction:    float64(env.Int("HISTORY_MIN_PERCENT", 50)) / 100,
		Tracing: telemetry.TracingConfig{

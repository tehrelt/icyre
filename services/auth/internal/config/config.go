// Package config loads Auth Service settings from the environment.
package config
...
type Config struct {
	...
	// SigningKeyID/SigningKey: Ed25519 seed (base64). Required outside local.
	SigningKeyID  string
	SigningKey    string
	AccessTTL     time.Duration
	SessionTTL    time.Duration
	SecureCookies bool
	// Login throttling: failures allowed per window per account.
	LoginMaxFailures int
	LoginWindow      time.Duration
}

// Local reports a developer environment.
func (c Config) Local() bool { return c.Env == "local" || c.Env == "test" }
...
func Load() (Config, error) {
	...
	cfg := Config{
		...
		SigningKeyID:     env.String("AUTH_SIGNING_KEY_ID", "icyre-dev-1"),
		SigningKey:       env.String("AUTH_SIGNING_KEY", ""),
		AccessTTL:        env.Duration("AUTH_ACCESS_TTL", 15*time.Minute),
		SessionTTL:       env.Duration("AUTH_SESSION_TTL", 30*24*time.Hour),
		LoginMaxFailures: env.Int("AUTH_LOGIN_MAX_FAILURES", 10),
		LoginWindow:      env.Duration("AUTH_LOGIN_WINDOW", 15*time.Minute),
		Tracing: telemetry.TracingConfig{
	...
	cfg.SecureCookies = env.Bool("AUTH_SECURE_COOKIES", !cfg.Local())
	if !cfg.Local() {
		env.RequiredString("AUTH_SIGNING_KEY")
	}
